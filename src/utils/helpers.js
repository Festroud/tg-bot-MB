...
async function handleAmhara(ctx) {
  const formattedText = `
<b>Дескрипторы</b>: 
<i>абрикос, карамель, черный чай</i>;
<b>Закладка кофе</b>: <i>17,4...18 грамм</i>;
<b>Время пролива</b>: - <i>21...30 сек</i>;
<b>Вес доппио</b>: — <i>32...36 грамм</i>;
  `;
  ctx.replyWithHTML(formattedText);
}

async function handleManager(ctx) {
  const formattedText = `
...
async function handlePomol(ctx) {
  const formattedText = `
<b>Настройка помола (эспрессо):</b>
1. Берем холдер и очищаем его от остатков кофе (протираем насухо).
2. Оттариваем холдер на весах (у кого новые гриндера, тогда этого делать не нужно, гриндер автоматически смалывает нужный нам вес)
